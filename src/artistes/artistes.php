<?php
    ...
    include '../bd_connection/bd_connection.php';$inputHTML = "<!DOCTYPE html>\n";
    ...
    $inputHTML .= "    <title>MusicPalace - Les Artistes</title>\n";
    $inputHTML .= "</head>\n";
    $inputHTML .= "<body>\n";

    $inputHTML .= $html."\n";

    $inputHTML .= "    <h1>LES ARTISTES</h1>\n";

    $connexion = OpenCon();
    $results = $connexion->query("SELECT DISTINCT CD_ARTIST FROM CD ORDER BY CD_ARTIST");
    $results->setFetchMode(PDO::FETCH_OBJ);
    while ($row = $results->fetch()) {
        $inputHTML .= "        <h2>$row->CD_ARTIST</h2>";
        $results2 = $connexion->query("SELECT * FROM CD WHERE CD_ARTIST = '$row->CD_ARTIST' ORDER BY CD_TITRE");
        ...
        while ($row2 = $results2->fetch()) {
            ...
            /**
             * @todo : Insérer le images sous forme réduites
             */
